...
export const changeStateByUser = (req,res) => {
    ...
    if(!state){
        return res.status(400).json({msg:'you should always provide a new state'})
    }
    
    Raspberry.findByPk(rbpid).then(raspberry => {
        delete state.price
        delete state.index
        console.log('state', state)
        raspberry.update({state : {...raspberry.state , ...state }}).then(rbpUpdated => {
...
export const changeStateByRaspberry = (req,res)=>{
    ...
    Raspberry.findByPk(id).then(raspberry => {
...
export const changePIByRaspberry = (req,res)=>{
    ...
    Raspberry.findByPk(id).then(raspberry => {
        ...
        raspberry.update({state : {...raspberry.state , price , index}}).then(rbpUpdated => {
            console.log('rbpUpdated', rbpUpdated.toJSON())
            return res.json({msg : 'success' , state : rbpUpdated.state})
        })
    }).catch(err => {
...
export const getStateR = (req, res)=>{
    ...
    Raspberry.findByPk(id , {include:User}).then(raspberry => {
        if(raspberry.password !== password){
            return res.status(400).json({msg : 'wrong password for engine'})
        }
        return res.json({msg : 'success' , state : raspberry.state , user : raspberry.User})
    }).catch(err => {
        return res.status(400).json({msg : 'something went wrong', err})
    })
}
...
export const getState = (req, res)=>{
    ...
    User.findByPk(id , {include : Raspberry}).then(user => {
        if(!user){
...
export const getRaspberryInfos = (req,res) => {
    ...
    if(!UserId){
        return res.status(400).json({msg : 'you should always provide the user Id'})
    }
